...
import { llmCallSkill, fileParserSkill, knowledgeRetrievalSkill, memoryInjectSkill } from '../skills/builtin-skills'

export class CodeGenAgent extends BaseAgent {
  readonly name        = 'codegen-agent'
  readonly description = '代码生成：根据 Spec 生成多语言代码'
  readonly domain      = '*'
  readonly skills: Skill[] = [llmCallSkill, fileParserSkill, knowledgeRetrievalSkill, memoryInjectSkill]
  ...
  async execute(ctx: AgentContext): Promise<AgentOutput> {
    ...
    const [retrieval, memory] = await Promise.all([
      knowledgeRetrievalSkill.execute(ctx, { spec }),
      memoryInjectSkill.execute(ctx, { keywords: [spec.title, ...(spec.entities || [])] })
    ])
    ...
    await Promise.all(langs.map(async (lang) => {
      ...
      const result = await llmCallSkill.execute(ctx, {
    ...
    if (outputs.length > 0) {
      try {
        ...
        await mem.saveSkillMemory(ctx.projectId, {
          ...
          template:    outputs.map((f: any) => f.path).join(', '),
...
function buildCodePrompt(spec: any, retrieval: any, memory: any, language: string, domain: any) {
  const fileRequirements = getFileRequirements(language, domain)
  const conventions      = [
    ...(retrieval.conventions || []),
    ...(domain.conventions    || []),
    ...(memory.memories?.slice(0, 2).map((m: any) => m.content.slice(0, 100)) || [])
  ].join('\n')

  const skillTemplates = memory.skills?.map((s: any) =>
    `// 参考技能: ${s.skill_name} - ${s.description}`
  ).join('\n') || ''
...
function getFileRequirements(language: string, domain: any): string {
