...
exports.SpecAnalysisAgent = void 0;
const agent_1 = require("../base/agent");
const builtin_skills_1 = require("../skills/builtin-skills");
class SpecAnalysisAgent extends agent_1.BaseAgent {
    constructor() {
        super(...arguments);
        this.name = 'spec-analysis-agent';
        this.description = '需求分析：将自然语言需求转化为结构化 Spec';
        this.domain = '*';
        this.skills = [builtin_skills_1.llmCallSkill, builtin_skills_1.memoryInjectSkill];
    }
    async execute(ctx) {
        const spec = ctx.spec;
        // 1. 从记忆注入历史相关经验
        const memory = await builtin_skills_1.memoryInjectSkill.execute(ctx, {
...
历史相关经验：
${memory.memories.slice(0, 3).map((m) => `- ${m.title}: ${m.content.slice(0, 100)}`).join('\n') || '无'}
...
${JSON.stringify(spec, null, 2)}`;
        const result = await builtin_skills_1.llmCallSkill.execute(ctx, { system, user, maxTokens: 2048 });
        let structuredSpec;
        try {
...
exports.SpecAnalysisAgent = SpecAnalysisAgent;
function buildDomainHints(domain) {
    const hints = {
...
//# sourceMappingURL=spec-agent.js.map