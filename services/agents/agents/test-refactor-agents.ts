...
import { BaseAgent, AgentContext, AgentOutput, Skill, GeneratedFile } from '../base/agent'
import { llmCallSkill, staticAnalysisSkill } from '../skills/builtin-skills'
...
export class TestAgent extends BaseAgent {
  ...
  readonly description = '自动化测试：运行测试套件，失败时触发 Auto-Fix（最多 3 次）'
  readonly domain      = '*'
  readonly skills: Skill[] = [staticAnalysisSkill]
  ...
  async execute(ctx: AgentContext): Promise<AgentOutput> {
    ...
    const files: GeneratedFile[] = codegenOutput?.files || []
    ...
    // 1. 静态分析（不需要编译器）
    const analysis = await staticAnalysisSkill.execute(ctx, { files })
  ...
  private failOutput(error: string): AgentOutput {
    return {
...
export class RefactorAgent extends BaseAgent {
  readonly name        = 'refactor-agent'
  readonly description = '代码重构：提取重复逻辑、优化命名、消除 Magic Number'
  readonly domain      = '*'
  readonly skills: Skill[] = [llmCallSkill, staticAnalysisSkill]
...
${files.map((f: any) => `### FILE: ${f.path}\n\`\`\`${f.language}\n${f.content}\n\`\`\``).join('\n\n')}`
    ...
    const result     = await llmCallSkill.execute(ctx, { system, user, maxTokens: 6144 })
    const { fileParserSkill } = require('../skills/builtin-skills')
    const refactored = await fileParserSkill.execute(ctx, { raw: result.content })

    // 合并：用重构后的覆盖原文件，其余保留
    const refMap     = new Map(refactored.files.map((f: any) => [f.path, f]))
    const finalFiles = files.map((f: any) => refMap.get(f.path) || f)
