...
exports.memoryInjectSkill = exports.knowledgeRetrievalSkill = exports.staticAnalysisSkill = exports.fileParserSkill = exports.llmCallSkill = void 0;
// ── LLM 调用 Skill（所有生成类 Agent 共用） ──────────────────
exports.llmCallSkill = {
    name: 'llm-call',
    ...
    async execute(_ctx, input) {
        ...
        for (const file of input.files) {
            ...
            if (file.language === 'go') {
                ...
                if (c.includes('panic('))
                    issues.push(`[${file.path}] 包含 panic()，应用层代码禁止使用`);
                const magic = c.match(/\b[0-9]{3,}\b/g);
                if (magic && magic.length > 3)
                    issues.push(`[${file.path}] 疑似 Magic Number: ${magic.slice(0, 3).join(',')}`);
            }
            if (file.language === 'typescript' || file.language === 'csharp') {
                if (c.split(': any').length > 4)
                    issues.push(`[${file.path}] 过多 any 类型`);
            }
            if (file.language === 'csharp') {
                if (!c.includes('namespace '))
                    issues.push(`[${file.path}] 缺少 namespace 声明`);
            }
        }
...
// ── 知识库检索 Skill ──────────────────────────────────────────
exports.knowledgeRetrievalSkill = {
...
function inferRole(path) {
    ...
    if (p.includes('_test') || p.includes('.test.') || p.includes('test.cs'))
        return 'test';
    if (p.includes('handler') || p.includes('controller'))
        return 'handler';
    if (p.includes('service'))
        return 'service';
    if (p.includes('model') || p.includes('entity') || p.includes('dto'))
        return 'model';
    if (p.includes('types') || p.includes('interface'))
        return 'types';
    if (p.includes('manager') || p.includes('presenter'))
        return 'client';
    return 'service';
}
