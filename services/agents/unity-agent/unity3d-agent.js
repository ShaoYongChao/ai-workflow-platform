...
class Unity3DAgent extends agent_1.BaseAgent {
    constructor() {
        super(...arguments);
        this.name = 'unity3d-agent';
        this.description = 'Unity3D 前端代码生成：C# Manager/Presenter/Model/Network/Test';
        this.domain = 'game';
        this.skills = [builtin_skills_1.llmCallSkill, builtin_skills_1.fileParserSkill, builtin_skills_1.knowledgeRetrievalSkill, builtin_skills_1.staticAnalysisSkill];
    }
    async execute(ctx) {
        const spec = ctx.spec;
        ...
        // 1. 知识库检索（优先 C# 相关接口）
        const retrieval = await builtin_skills_1.knowledgeRetrievalSkill.execute(ctx, { spec: finalSpec });
        // 2. 服务端输出（如果有，用于对齐接口）
        const serverFiles = ctx.prevOutputs['codegen-agent']?.files?.filter((f) => f.language === 'go') || [];
        ...
        const result = await builtin_skills_1.llmCallSkill.execute(ctx, { system, user, maxTokens: 10240 });
        const parsed = await builtin_skills_1.fileParserSkill.execute(ctx, { raw: result.content });
        // 4. 静态分析 C# 文件
        const analysis = await builtin_skills_1.staticAnalysisSkill.execute(ctx, { files: parsed.files });
...
exports.Unity3DAgent = Unity3DAgent;
// ── Unity C# Prompt 构建 ──────────────────────────────────────
function buildUnityPrompt(spec, retrieval, apiContracts) {
...
${apiContracts.length > 0
        ? apiContracts.join('\n')
        : spec.api_contract?.map((a) => `${a.type} /api/${featureFolder}/${a.name}`).join('\n')}
...
function extractAPIContracts(serverFiles, spec) {
    const contracts = [];
    for (const file of serverFiles) {
        if (file.role !== 'handler')
            continue;
        const handlerLines = file.content.split('\n').filter((l) => l.includes('HandleFunc') || l.includes('func (h *Handler)') || l.includes('mux.Handle'));
        contracts.push(...handlerLines.map((l) => `// 服务端: ${l.trim()}`));
    }
    // fallback 使用 spec 中的 api_contract
    if (contracts.length === 0) {
        const featureFolder = toSnakeCase(spec.title);
        return (spec.api_contract || []).map((a) => `${a.type} /api/${featureFolder}/${a.name}`);
    }
...
//# sourceMappingURL=unity3d-agent.js.map